...
const PortfolioPDFButton = () => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      tempContainer.className = 'portfolio-pdf-container';
      ...
      // 複製 Portfolio 內容
      const clonedContent = portfolioContent.cloneNode(true) as HTMLElement;
      
      // 移除不需要的元素（如按鈕、互動元素等）
      const elementsToRemove = clonedContent.querySelectorAll('button, .hover\\:shadow-xl, .transition-all, .cursor-pointer, .hover\\:scale-105');
      ...
      // PDF 配置選項 - 針對 Portfolio 優化
      const pdfOptions = {
        margin: [10, 10, 10, 10],
        filename: 'Wilson_Ho_Portfolio.pdf',
      ...
      toast({
        title: "Portfolio PDF 生成成功！",
        description: "你的作品集已成功下載為 PDF 檔案。",
        variant: "default",
      });
